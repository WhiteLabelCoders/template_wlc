...
class Maats_Overview extends Abstract_Block {
	...
	public function render_frontend( array $block, $content = '', $is_preview = false, $post_id = 0 ) {
		...
		<section id="<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $class ); ?>">
			<div class="container-wide maats-overview--content">
				<?php if ( isset( $content['heading'] ) ) : ?>
					<h2 class="maats-overview--content--heading heading"><?php echo esc_html( $content['heading'] ); ?></h2>
				<?php endif; ?>
				<?php if ( $maats ) : ?>
					<div class="maats-overview--content--maats">
						<?php foreach ( $maats as $maat ) : ?>
							<a class="maats-overview--maat-item" href="<?php echo esc_url( $maat['url'] ); ?>">
								<?php if ( $maat['thumbnail'] ) : ?>
									<figure class="maats-overview--maat-item--thumbnail">
										<?php echo wp_kses_post( $maat['thumbnail'] ); ?>
									</figure>
								<?php endif; ?>
								<h3 class="maats-overview--maat-item--title"><?php echo esc_html( $maat['title'] ); ?></h3>
								<?php if ( $maat['post_excerpt'] ) : ?>
									<p class="maats-overview--maat-item--excerpt">
